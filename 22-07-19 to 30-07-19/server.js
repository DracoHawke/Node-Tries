...
var url = require('url');
insert_db=require('./assets/javascript/insert_db');
login=require('./assets/javascript/login');
var db_connect = require('./assets/javascript/db-connect');
confirmation=require('./assets/javascript/confirmation');
sitter=require('./assets/javascript/sittersignup');
var bcrypt = require('bcryptjs');
var formvalidator = require('./assets/javascript/formvalidator');
var dformvalidator = require('./assets/javascript/dformvalidator');
  ...
  if(flag == 1){ // check for validation flag. if true:
    ...
    connection.query('SELECT users.Uid,users.Fname,users.status,users.U_password from users where users.Email = "' + c.u_email +'"', function(err, rows, fields) {
      ...
      if(rows.length > 0){ // if any rows returned. if true(user already registered):
        ...
        console.log('newest');
        let insertQuery = 'INSERT INTO ??(Fname,Lname,Email,U_password) VALUES (?,?,?,?)';
        let query = mysql.format(insertQuery,["users",c.u_fname,c.u_lname,c.u_email,c.u_pass]);
        connection.query(query,(err, response) => {
            if(err) {
                console.error(err);
                flags = 4060; // server database connectivity error;
