...
module.exports = function(req,res,count){
  if(req.session.admin_name){
    var sql = "SELECT count(`users`.`Uid`) AS `users`, count(`sitters`.`Sid`) AS `sitters`, count(DISTINCT `dogs`.`Uid`) AS `dogs`FROM `users` LEFT JOIN `sitters` ON `sitters`.`Uid` = `users`.`Uid` LEFT JOIN `dogs` ON `dogs`.`Uid` = `users`.`Uid`";
    connection.query(sql, function(err, rows, fields) {
      if(err) throw err;
      var count_admin = rows;
      count_admin[0].count = count;
      var sql = "SELECT count(`dogs`.`Uid`) as dogs FROM `dogs`";
      connection.query(sql, function(err, rows, feilds){
        count_admin[0].no_of_dogs = rows[0].dogs;
        ...
        connection.query(sql, function(err, rows, fields) {
          ...
          connection.query(sql, function(err, rows, fields) {
            ...
            var sql = "SELECT `Did`, `DogName`, `DogBreed`, `DogGender`, `DogAge`, `DogPic1`, `DogPic2`, `DogPic3`, `DogPic4`, `DogPic5`, `Uid` FROM `dogs` WHERE AdminStatus=0 ORDER BY Did DESC LIMIT 5";
            connection.query(sql, function(err, rows, fields) {
              var dog_ad = rows;
              ...
              connection.query(sql, function(err, rows, fields) {
                ...
                res.render('admin',{details:req.session, count:count_admin, sit_ad:sit_ad, dog_ad:dog_ad, url1: req.url});
              });
            });
          });
        });
