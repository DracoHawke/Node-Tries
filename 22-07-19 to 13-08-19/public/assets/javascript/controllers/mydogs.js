...
module.exports=function(req,res,error1){
  console.log("ccurrently in my dogs")
  var sid = 0;
  var did = 0;
    ...
    if(req.session.status == 0){
      ...
      con.query(sql, function (err, rows, fields) {
        ...
        if(send_data.Did != null)
          res.render('dashboard',{uname:req.session.uname, sid:sid, did:did, send_data:send_data, error:error1, check: "mydogs", login:req.session});
        else
          res.render('dashboard',{uname:req.session.uname, sid:sid, did:did, send_data:send_data, error:error1, check: "mydogs", login:req.session});
      });
    }
