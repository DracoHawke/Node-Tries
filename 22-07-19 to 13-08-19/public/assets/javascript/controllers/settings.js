...
module.exports=function(req,res,error1){
  ...
  if(req.session.uname){
    ...
    con.query(sql, function (err, rows, fields) {
      ...
      if(rows.length == 0){
        res.render('dashboard',{uname:req.session.uname,sid:sid,did:did,send_data:{status_err:'not ver',file:''},error:{},check:"", adms: "no", login:req.session});
      }
      else if(rows.length == 1){
        send_data = rows[0];
        send_data.status_err = "verified";
        if(send_data.AdminStatus == 0){
          res.render('dashboard', {uname:req.session.uname, sid:sid, did:did, send_data:send_data, error:error1, check: "settings", adms: "no", login:req.session});
        }
        else if(send_data.Location == null){
          res.render('dashboard', {uname:req.session.uname, sid:sid, did:did, send_data:send_data, error:error1, check: "settings", adms: "yes", login:req.session});
        }
