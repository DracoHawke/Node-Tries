...
module.exports = function(app) {
  ...
  app.get('/about',urlencodedParser,function(req,res){
    if(req.session.uname){
      ...
      var uname = ' ';
    res.render('about',{uname:uname,did: did,sid:sid,login:req.session});
  });
  ...
  app.get('/faq',urlencodedParser,function(req,res){
    if(req.session.uname){
      ...
      var uname = ' ';
    res.render('faq',{uname:uname,did:did,sid:sid,login:req.session});
  });
  ...
  app.get('/blog',urlencodedParser,function(req,res){
    if(req.session.uname){
      ...
      var uname = ' ';
    res.render('blog',{uname:uname,did:did,sid:sid,login:req.session});
  });
        ...
        if(rows.length > 0){ // if any rows returned. if true(user already registered):
          ...
          if(flags == 1 || flags == 10 || flags == 20){
            ...
            if(req.session.status != 0){
              res.render('success',{uname : req.session.uname, data: "", status: req.session.status, sid: req.session.sid, did: req.session.did,login:req.session});
            }
            else{
  ...
  app.get('/dashboard/:page',function(req,res){
    ...
    var error1 = {};
    console.log("in get with page: ",page);
    ...
    if("location" == page){
      ...
      if(typeof req.body.password !== "undefined"){
        ...
        if(req.body.password == ""){
          ...
          console.log("yes: ",req.body.password," yes");
