...
module.exports=function(req,res){
  if(req.session.admin_name){
    if(req.query.Did){
      ...
      var sql = 'SELECT `users`.`Uid`, `users`.`Fname`, `users`.`Lname`, `users`.`Email`, `users`.`Phone`, `users`.`status`, `users`.`No_of_Dogs`, `users`.`Profile`,`sitters`.* FROM `users` LEFT JOIN `sitters` ON `sitters`.`Uid` = `users`.`Uid` WHERE `users`.`Uid`='+mysql.escape(req.query.Uid);
      connection.query(sql, function(err, rows, fields) {
        if(err) throw err;
        var user_det = rows;
