...
module.exports=function(req,res){
  if(req.session.admin_name){
    ...
    var sql = "SELECT `dogs`.`DogName`,`dogs`.`Did`,`dogs`.`DogBreed`,`dogs`.`DogGender`,`dogs`.`DogAge`,`users`.`Email`,`users`.`status`,`dogs`.`DogPic1`,`dogs`.`DogPic2`,`dogs`.`DogPic3`,`dogs`.`DogPic4`,`dogs`.`DogPic5`,`dogs`.`AdminStatus` FROM `users` INNER JOIN `dogs` ON `dogs`.`Uid` = `users`.`Uid` LIMIT 4 OFFSET "+off;
  //  console.log(sql);
    connection.query(sql, function(err, rows, fields) {
      if(err) throw err;
      var alldog=rows;
