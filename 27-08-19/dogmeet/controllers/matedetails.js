...
module.exports = function(req,res){
  if(!req.query.id){
    ...
    if(req.query.stars) {
      ...
      var sql = "SELECT `dogs`.`Did`,`dogs`.`Rating`,`dogs`.`Reviews`,`dogs`.`Description`, `dogs`.`DogName`, `dogs`.`DogBreed`, `users`.`Fname`,`dogs`.`DogAge`, `users`.`Lname`,`users`.`Email`,`users`.`Phone`,`users`.`Profile`, `dogs`.`DogPic1`,`dogs`.`DogPic2`,`dogs`.`DogPic3`,`dogs`.`DogPic4`,`dogs`.`DogPic5` FROM `users` INNER JOIN `dogs` ON `dogs`.`Uid` = `users`.`Uid` where `dogs`.`AdminStatus`=1 and `users`.`status`=1 and `users`.`Email`="+mysql.escape(req.query.id)+" and `dogs`.`enabled` = 1 and `dogs`.`Did` = "+mysql.escape(req.query.did);
      console.log("sql: ",sql);
      con.query(sql, function (err, rows, fields) {
        if(req.session.sid)
