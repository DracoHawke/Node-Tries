...
module.exports=function(req,res,err1,c){
  ...
  if(req.session.uname){
    if(req.session.status == 0){
      ...
      if(rows.length == 0){
          res.render('registerdog', {uname: req.session.uname ,data: data, status: req.session.status, sid: "", did: '',set: "", login:req.session, c:c});
        }
        else{
          if(rows[0].status == 0) {
            res.render('AuthenticationNeeded',{uname : req.session.uname, data: data, status: req.session.status,sid:sid , did: did, login:req.session});
            ...
            res.render('registerdog', {uname: req.session.uname, data: data, sid: sid, did: did, set: set1, status: req.session.status, nod: no_of_dogs, login:req.session, c:c});
          }
        }
      });
    }
