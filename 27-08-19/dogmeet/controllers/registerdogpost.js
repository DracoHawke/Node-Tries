      ...
      if(rows.length > 0){ // if any rows returned. if true(user already registered):
        ...
        if(flags == 1 || flags == 10 || flags == 20){
          ...
          while(i < nod2) {
            ...
            d = dog[i];
            e = d['files'];
            //console.log(d);console.log("c ", c);console.log(e);
            if(e.name){
              //console.log("yess");
              l = 1;
            }
          ...
          //console.log("curr flag: ",flags);
        }
      }
      else if(rows.length == 0){
        //console.log('newest');
        let insertQuery = 'INSERT INTO ??(Fname,Lname,Email,U_password,Phone,Profile,status,No_of_Dogs) VALUES (?,?,?,?,?,?,?,?)';
        let query = mysql.format(insertQuery,["users",c.u_fname,c.u_lname,c.u_email,bcrypt.hashSync(c.u_pass, 10),c.u_phone,'/users/default.png',0,nod2]);
            ...
            if(err) {
              ...
              connection.query('select users.Uid from users where users.Email = "'+c.u_email+'"',function(err,rows3,feilds){
                ...
                if(flags == 1 || flags == 3 || flags == 10){
                  var i = 0;
                  //console.log('dog');console.log(nod2);console.log(dog);
                  var dpic = [];
                  while(i < nod2) {
                    //console.log("dogs1");
                    d = dog[i];
