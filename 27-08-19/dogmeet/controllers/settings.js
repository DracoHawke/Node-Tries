    ...
    con.query(sql, function (err, rows, fields) {
      ...
      if(rows.length == 0){
        ...
        if(send_data.AdminStatus == 0){
          res.render('dashboard', {uname:req.session.uname, sid:sid, did:did, send_data:send_data, error:error1, page: "settings", adms: "no", login:req.session});
        }
        else if(send_data.Location == null){
          res.render('dashboard', {uname:req.session.uname, sid:sid, did:did, send_data:send_data, error:error1, page: "settings", adms: "yes", login:req.session});
        }
