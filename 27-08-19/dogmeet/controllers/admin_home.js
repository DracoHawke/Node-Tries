    ...
    connection.query(sql, function(err, rows, fields) {
      ...
      connection.query(sql, function(err, rows, fields) {
        ...
        connection.query(sql, function(err, rows, fields) {
          ...
          connection.query(sql, function(err, rows, fields) {
            var dog_ad=rows;
            var sql="SELECT `sitters`.`Uid`,`sitters`.`Sid`,`users`.`Fname`,`users`.`Email`, `users`.`Lname`,`users`.`Profile`,`sitters`.`DOB`,`users`.`status` FROM `users` INNER JOIN `sitters` ON `sitters`.`Uid` = `users`.`Uid` where sitters.AdminStatus=0 ORDER By sitters.Sid DESC LIMIT 5";
            connection.query(sql, function(err, rows, fields) {
              var sit_ad=rows;
              //console.log(dog_ad);console.log(sit_ad);console.log(req.session);
