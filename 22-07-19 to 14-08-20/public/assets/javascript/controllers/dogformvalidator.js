...
const schema_breed = Joi.object().keys({dog_breed: Joi.string().regex(/^[a-zA-Z_\-]*$/).required()})
const schema_gender = Joi.object().keys({dog_gender: Joi.string().valid(['Male','Female']).required()})
const schema_age = Joi.object().keys({dog_age:Joi.string().regex(/^([0-9]\.[0-9])$|^(1[0-9].[0-9])$|^([0-9])$|^(1[0-9])$/).required()})
