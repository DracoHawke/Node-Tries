    ...
    connection.query(sql, function(err, rows, fields) {
      ...
      if(req.session.status == 0){
        ...
        if(req.query.to) {
            ...
            connection.query(sql, function(err, rows, fields) {
              ...
              var sql = 'UPDATE `messages` SET `read`=1 WHERE toEmail='+mysql.escape(req.session.email)+' and Email='+mysql.escape(req.query.to);
                ...
                connection.query(sql, function (err, rows, fields) {
                  if(err) throw err;
                  var to_det = rows;
                  res.render('dashboard',{ChatList:ChatList, login:req.session, uname:uname, send_data:'', sid:sid, did:did, check:page, to:req.query.to, all_mess:all_mess, to_det:to_det});
                });
              });
            });
          }
          ...
          res.render('dashboard',{ChatList:ChatList, login:req.session, uname:uname, send_data:'', sid:sid, did:did, check:page, to:''});
      }
    });
  }
  else{
