...
module.exports = function(app){
  ...
  app.post('/savetemp/:id',urlFile,function(req,res) {
    ...
    //console.log(data);
    data = data + "<div class='row'><div class='col-md-12'><button type='submit' class='btn-theme btn-submit'>Submit</button><button type='reset' class='btn-danger btn-reset' value='Reset'>Reset</button></div></div>";
  ...
  app.post('/createnewlink',urlFile,function(req,res){
    ...
    route_name = route_name.replace(/[ \/\\\_\!\:\;\#\&\*\(\)\^\@\$\%\+\=\[\]\;\"\?\>\<\{\}]/gmi, "");
    if(route_name.length < 2) {
      route_name = "route_"+route_name;
    }
    else if(route_name.length > 50) {
      route_name = route_name.substr(0,48);
    }
  ...
  app.get('/getknownroutes',function(req,res) {
    console.log("in getknownroutes");
    getknownroutes(req,res);
  })

  app.get('/getknownroutes/:id',function(req,res) {
    console.log(req.params.id);
    var url = mysql.escape(req.params.id);
    url = url.substr(1,url.length-2);
    console.log(url);
    url = "'/"+url+"'";
    var sql = "SELECT * FROM `routes` WHERE `routes`.`Route` = "+ url;
    console.log(sql);
