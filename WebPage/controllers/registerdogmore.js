...
module.exports=function(req,res){
  ...
  if(req.session.uname){
    if(req.session.status == 0){
      ...
      console.log(sid);
      res.render('AuthenticationNeeded',{uname : req.session.uname, data: "", status: req.session.status,sid:sid , did: did, login:req.session});
    }
    else{
      ...
      con.query(sql, function (err, rows, fields) {
        if(rows.length == 0){
          res.render('registerdog', {uname: req.session.uname ,data: "", status: req.session.status, sid: "", did: '',set: "", login:req.session});
        }
        else{
          if(rows[0].status == 0) {
            res.render('AuthenticationNeeded',{uname : req.session.uname, data: "", status: req.session.status,sid:sid , did: did, login:req.session});
