  ...
  if(req.query.stars){
    ...
    con.query(sql, function (err, rows, fields) {
      ...
      con.query(sql, function (err, rows, fields) {
        ...
        if(rows.length!=0)
          res.render('sitterdetails',{uname:uname,sid:sid,rows:details,update:rows[0].rating,login:req.session});
        else
          res.render('sitterdetails',{uname:uname,sid:sid,rows:details,update:'',login:req.session});
      });
    });
  }
}
