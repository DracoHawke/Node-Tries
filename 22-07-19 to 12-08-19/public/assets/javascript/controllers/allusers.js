...
module.exports=function(req,res){
  if(req.session.admin_name){
    ...
    var sql="SELECT `users`.`Fname`,`users`.`Uid`,`users`.`Lname`,`users`.`Email`,`users`.`Phone`,`users`.`status`,`users`.`Profile`,`sitters`.`Sid`,`dogs`.`Did` FROM `users` LEFT JOIN `dogs` ON `dogs`.`Uid` = `users`.`Uid` LEFT JOIN `sitters` ON `sitters`.`Uid` = `users`.`Uid` LIMIT 4 OFFSET "+off;
  //  console.log(sql);
