    ...
    if(req.session.status == 0){
      ...
      con.query(sql, function (err, rows, fields) {
        ...
        if(send_data[0].Did != null)
          res.render('dashboard',{uname:req.session.uname, sid:sid, did:did, send_data:send_data, error:error1, check:"dogdetails", dno: dno,login:req.session});
        else{
          console.log("no did", error1);
          res.render('dashboard',{uname:req.session.uname, sid:sid, did:did, send_data:send_data, error:error1, check: "dogdetails", dno: dno,login:req.session});
        }
      });
    }
