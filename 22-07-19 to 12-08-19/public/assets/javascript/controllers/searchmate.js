...
module.exports = function(req,res,error1){
  ...
  if(ageflag == 1){
    var sql = "SELECT `dogs`.`DogAge`, `dogs`.`Did`, `dogs`.`DogName`, `dogs`.`Rating`, `dogs`.`Reviews`, `dogs`.`Description`, `users`.`Fname`, `users`.`Lname`, `users`.`Email`, `dogs`.`DogPic1`, `dogs`.`DogPic2`, `dogs`.`DogPic3`, `dogs`.`DogPic4`, `dogs`.`DogPic5`, `dogs`.`DogBreed`, `users`.`status` FROM `users` INNER JOIN `dogs` ON `dogs`.`Uid` = `users`.`Uid` where `dogs`.`AdminStatus` = 1 and `users`.`status` = 1 and (`dogs`.`DogBreed` LIKE '%"+ a +"%' OR  DogAge BETWEEN "+b1+" AND "+b2+") order by `dogs`.`"+sb+"` DESC LIMIT "+off+",4";
  ...
  con.query(sql, function (err, rows, fields) {
    ...
    con.query(sql2, function(err, rows2, fields) {
      ...
      res.render('findmate',{uname: uname, url:req.query, sid: sid, rows: alldogs, totaldogs: totaldogs, did: did, status: req.session.status, search: 1, c: c, login:req.session});
    });
  });
};
