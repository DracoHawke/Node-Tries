...
module.exports = function(req, res, check, send_data) {
  ...
  if(check == "single") {
    geocoder.geocode(req.query.Location, function(err, response) {
      ...
      "and `users`.`status` = 1 HAVING distance < `sitters`.`Radius` order by `sitters`.`Rating` DESC LIMIT "+off+",4";
      ...
      con.query(sql, function (err, rows, fields) {
        ...
        con.query(sql2, function(err, rows2, fields) {
          ...
          c['location'] = req.query.Location;
          console.log(c);
          res.render('findsitter', {uname: uname, url: req.query, sid: sid, rows: allsitters, did: did, totalsitters: totalsitters, status: req.session.status, rowlen: rows.length, filters: "yes", c: c, login:req.session});
        });
      });
    });
  }
